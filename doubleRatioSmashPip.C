...
int doubleRatioSmashPip()
{
	...
	TFile *f_exp = new TFile("./jobs/wires/analysisPip_2sigma_wires_Buffer10_NoIsGoodEvent_nTracksGT1_-130VertexZ-110_maxDistTo0XYis5_notMdcEdge_noDoubleHits_VertexZ1mmMetaHitsMix_WireDistanceGT4NoSharedMetaAllLayers_beamTiltCorr_gen4_50k.root","read");//experiment
	TFile *f_sim = new TFile("./jobs/smash/analysisSmashPip_Buffer20_NoIsGoodEvent_nTracksGT1_-130VertexZ-100_maxDistTo0XYis5_VertexZ1mmMetaHitsMix_WireDistanceGT4NoSharedMetaAllLayers_50k_gen4.root","read");//simulation
	...
	for(int i=1; i<10; i++)
	{
		for(int j=1; j<10; j++)
		{
			...
			if(tempSigExp && tempsigSMASH)
			{
				...
				canvas.cd();
				canvas.Clear();
				tempBckgExp->SetNameTitle("pipBckg","pipBckg");
